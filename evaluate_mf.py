...
@torch.no_grad()
def create_spring_submission_mf(args, model, iters, output_path='sintel_submission', nframes=4):
    """ Create submission for the Sintel leaderboard """
    model.eval()

    test_dataset = mf_datasets.SpringSubmission(root=os.path.join(args.spring_root, 'test'), input_frames=nframes)

    for test_id in tqdm(range(len(test_dataset))):
        images, (scene, direction, cam, frame_ids) = test_dataset[test_id]
        ...
        for i in range(len(flows)):
            if frame_ids[i] != -1:
                output_file = os.path.join(output_dir, f'flow_{direction}_{cam}_%04d.flo5' % (frame_ids[i]))
                frame_utils.writeFlo5File(flows[i], output_file)

@torch.no_grad()
def validate_spring_mf(model, iters=6, root='/data/Sintel', tqdm_miniters=1, nframes=3, split=False):
    """ Peform validation using the Sintel (train) split """
    model.eval()
    val_dataset = mf_datasets.SpringEval(input_frames=nframes, forward_warp=False, root=os.path.join(root, 'train'), subsample_groundtruth=True, split=split)
    epe_list = []
    epe_list_10 = []
    epe_list_10_40 = []
    epe_list_40 = []
    ...
    for val_id in tqdm(range(len(val_dataset))):
        images, flows_gt, _, (folder_name, frame_ids) = val_dataset[val_id]
        
        valid = [~torch.isnan(torch.sum(flows_gt[i], dim=0)) for i in range(len(flows_gt))]
        valid_10 = [valid[i] & (torch.sum(flows_gt[i] ** 2, dim=0).sqrt() < 10) for i in range(len(flows_gt))]
        valid_10_40 = [valid[i] & (torch.sum(flows_gt[i] ** 2, dim=0).sqrt() >= 10) & (torch.sum(flows_gt[i] ** 2, dim=0).sqrt() < 40) for i in range(len(flows_gt))]
        valid_40 = [valid[i] & (torch.sum(flows_gt[i] ** 2, dim=0).sqrt() >= 40) for i in range(len(flows_gt))]
        ...
        for i in range(len(flows)):
            ...
            if frame_ids[i] != -1:
                ...
                epe_list_10.append(epe.view(-1)[valid_10[i].view(-1)].numpy())
                epe_list_10_40.append(epe.view(-1)[valid_10_40[i].view(-1)].numpy())
                epe_list_40.append(epe.view(-1)[valid_40[i].view(-1)].numpy())
    ...
    print("Spring 1px: %f, 1px(s0~10): %f, 1px(s10~40): %f, 1px(s40+): %f" % (px1_x, px1_10, px1_10_40, px1_40))
...
@torch.no_grad()
def validate_sintel_mf_stride2(model, iters=6, root='/data/Sintel', tqdm_miniters=1, nframes=3):
    """ Peform validation using the Sintel (train) split """
    model.eval()
    results = {}
    for dstype in ['clean', 'final']:
        val_dataset = mf_datasets.SintelMultiframeEval_stride2(split='training', dstype=dstype, root=root, nframes=nframes)
        epe_list = []

        predicted_frame = []
        for val_id in tqdm(range(len(val_dataset)), miniters=tqdm_miniters):
            images, flows_gt, _, (base_folder, frame_ids) = val_dataset[val_id]
            images = [x[None].cuda() for x in images]
            ...
            for i in range(len(flows)):
                # print(len(flows), flows[i].shape, flows_gt[i].shape)
                if (base_folder + str(frame_ids[i])) in predicted_frame: continue
                else:
                    predicted_frame.append([base_folder + str(frame_ids[i])])
                    epe = torch.sum((flows[i] - flows_gt[i])**2, dim=0).sqrt()
                    epe_list.append(epe.view(-1).numpy())
                    # print(val_id, frame_ids, epe.mean().item())
...
@torch.no_grad()
def create_kitti_submission_mf(model, iters, output_path='kitti_submission'):
    """ Create submission for the Sintel leaderboard """
    model.eval()
    test_dataset = datasets.KITTI(split='testing', aug_params=None) # 和sintel submission的dataset有何处不同
    ...
    for test_id in range(len(test_dataset)):
        ...
        images = [x[None].cuda() for x in images]
        padder = InputPadder(images[0].shape, mode='kitti') # TODO: Mode?
        images = padder.pad_list(images)
        flows = model(images, iters=iters, test_mode=True)
        flows = [padder.unpad(flow[0]).permute(1, 2, 0).cpu().numpy() for flow in flows]
        ...
        image.save(f'vis_kitti/flow/{frame_name}')
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        # create_sintel_submission_mf(args, model.module, args.iters, output_path=args.output_path, nframes=args.T)
        # create_sintel_submission_mf_warmup(args, model.module, args.iters, output_path=args.output_path, nframes=args.T)

        # validate_spring_mf(model.module, iters=15, root=args.spring_root, nframes=4, split=False)
        # create_spring_submission_mf(args, model, iters=15, output_path='/userhome/submission-' + os.path.basename(args.model)[:-4], nframes=4)
