...
class ResidualBlock3D(nn.Module):
    def __init__(self, in_planes, planes, norm_fn='group', stride=1, drop_path=0., T=3):
        super().__init__()
...
class Res18_3D(nn.Module):
    def __init__(self, args, norm_fn='batch'):
        ...
        self.layer1 = self._make_layer(64, stride=1, drop_path0=drop_rate[0], drop_path1=drop_rate[1], T=args.T)
        self.layer2 = self._make_layer(96, stride=2, drop_path0=drop_rate[2], drop_path1=drop_rate[3], T=args.T)
        self.layer3 = self._make_layer(128, stride=2, drop_path0=drop_rate[4], drop_path1=drop_rate[5], T=args.T)
    ...
    def _make_layer(self, dim, stride=1, drop_path0=0., drop_path1=0., T=3):
        layer1 = ResidualBlock3D(self.in_planes, dim, self.norm_fn, stride=stride, drop_path=drop_path0, T=T)
        layer2 = ResidualBlock3D(dim, dim, self.norm_fn, stride=1, drop_path=drop_path1, T=T)
        layers = (layer1, layer2)
...
class AttnBasicEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        self_attn = rearrange(self_attn, 'b h x y u v -> b h (x y) (u v)').softmax(dim=-1)
