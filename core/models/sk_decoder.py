...
def vis_featmap(x, save_prefix):
    plt.imshow(x.cpu().squeeze(0).mean(0), cmap='Greens')
    plt.savefig(save_prefix +'.png')
    plt.imshow(x.cpu().squeeze(0).max(0)[0], cmap='Greens')
    plt.savefig(save_prefix+'-max.png')
...
class SK_Decoder(nn.Module):
    def __init__(self, args):
        ...
        elif args.cost_encoder_v3:
            self.cost_memory_encoder = eval(args.SKII_Block)(256, 128, [3], args)
            self.cost_global_encoder = CostGlobalEncoder(args)
            self.to_k = nn.Conv2d(324, 128, 1)
            self.to_v = nn.Conv2d(324, 128, 1)
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, upsample=True, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision):
            ...
            net = torch.tanh(net)
            inp = torch.relu(inp)
            attention = self.att(inp) if self.att is not None else None
        ...
        for itr in range(iters):
            ...
            vis_featmap(corr, '/share/sunshangkun/sskflow/vis/corr_before')

            if self.args.cost_encoder_v2:
                corr = self.cost_encoder(corr)
            elif self.args.cost_encoder_v1 or self.args.cost_encoder_v3:
            ...
            vis_featmap(corr, '/share/sunshangkun/sskflow/vis/corr_after')
